...
const storage = multer.diskStorage({
  destination: (req, file, cb) => { const tempDir = path.join("uploads","temp"); fs.ensureDirSync(tempDir); cb(null,tempDir); },
...
app.post("/api/build", upload.fields([{ name:"icon"},{name:"splash"}]), async (req,res)=>{
  try {
    ...
    exec(`cd ${tempBuild} && ./gradlew assembleRelease`, (err)=>{
      if(err){ console.error(err); return res.status(500).json({success:false,message:"APK build failed"}); }

      const apkPath = path.join(tempBuild,"app/build/outputs/apk/release/app-release.apk");
      const buildDir = path.join("uploads","builds"); fs.ensureDirSync(buildDir);
